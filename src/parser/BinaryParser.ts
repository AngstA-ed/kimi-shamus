...
export class BinaryParser {
  private readonly SEGMENT_SIZE = 128;
  private readonly MAZE_SIZE = 640; // 5 × 128
  private readonly TOTAL_ROOMS = 128;
  ...
  parseMapSegment(buffer: Uint8Array, mazeType: MazeType): ParsedMaze {
    ...
    for (let roomId = 0; roomId < this.TOTAL_ROOMS; roomId++) {
      const room = this.constructRoom(
        roomId,
        rawData.segment1[roomId],
        rawData.segment2[roomId],
        rawData.segment3[roomId],
        rawData.segment4[roomId],
        podRooms,
        levelBoundaries,
        warnings
      );
      ...
      if (room.isCorridor) stats.corridorCount++;
      if (room.object) stats.objectCount++;
    }
   ...
   */
  private decodeSegment1(byte: number): {
   ...
   * $06 = The Shadow
   */
  private decodeSegment3(byte: number): ObjectType | undefined {
    switch (byte) {
  ...
  private constructRoom(
    ...
    const roomObject: RoomObject | undefined = s3 ? {
      type: s3,
      color: s4.color,
      luma: s4.luma,
   ...
   */
  private classifyCorridor(value: number): CorridorType {
   ...
   */
  private getHorizontalWallPosition(bit: number): { x: number; y: number } {
   ...
   */
  private getVerticalWallPosition(bit: number): { x: number; y: number } {
   ...
   */
  private generateEnemies(roomId: number, boundaries: LevelBoundaries): EnemySpawn[] {
   ...
   */
  private getEnemyCountForLevel(level: LevelColor): number {
    switch (level) {
      case 'BLACK': return 2;
      case 'BLUE': return 3;
      case 'GREEN': return 4;
