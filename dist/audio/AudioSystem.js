...
export class AudioSystem {
    constructor() {
        this.ctx = null;
        this.currentMazeType = MazeType.ORIGINAL_ATARI;
     ...
     */
    playAtariPing() {
        if (!this.ctx || !this.config.atariPingEnabled)
            return;
     ...
     */
    playWarning() {
        if (!this.ctx || !this.config.warningEnabled)
            return;
        const osc = this.ctx.createOscillator();
        const gain = this.ctx.createGain();
     ...
     */
    startBackgroundHum() {
        if (!this.ctx || !this.config.backgroundHumEnabled || this.config.c64SilentMode)
            return;
        // Stop existing hum
        this.stopBackgroundHum();
