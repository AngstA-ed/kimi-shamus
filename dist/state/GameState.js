 ...
 */
import { MazeType, TOURNAMENT_SEQUENCE } from '../types';
/**
...
export class GameState {
     ...
     */
    getBaseSpeedForDifficulty(difficulty) {
        switch (difficulty) {
     ...
     */
    increaseSpeedAfterClear() {
        // Decrease speed value = increase actual speed
        this.state.shamusSpeed = Math.max(0, this.state.shamusSpeed - DEFAULT_CONFIG.SPEED_BOOST_AFTER_CLEAR);
    }
    /**
     * Update speed when changing levels
     * Based on $0206 register logic from Shamuspl.asm
     */
    updateSpeedForLevel(level, direction) {
        if (direction === 'up') {
     ...
     */
    reset(mazeType = MazeType.ORIGINAL_ATARI) {
